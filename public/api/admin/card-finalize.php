...
$st = $pdo->prepare('SELECT id, name FROM ts_cards WHERE id=? LIMIT 1');
$st->execute([$id]);
$card = $st->fetch();
if (!$card) ts_json_error(404, 'کارت یافت نشد');

if ($mode === 'reset') {
    $pdo->prepare('UPDATE ts_cards SET display_balance_usd=NULL, finalized_at=NULL, updated_at=? WHERE id=?')
        ->execute([date('Y-m-d H:i:s'), $id]);
    ts_card_alloc_log(
        $id, null, 'card_balance', null, null, 'USD', null,
        'لغو اتمام کارت «' . $card['name'] . '» — بازگشت به نمایش سقف اصلی'
    );
    ts_json(200, ['ok' => true, 'display_balance_usd' => null, 'finalized_at' => null]);
}

// finalize: snapshot sum of kotaj USD for this card
...
$pdo->prepare('UPDATE ts_cards SET display_balance_usd=?, finalized_at=?, updated_at=? WHERE id=?')
    ->execute([$sumUsd, $now, $now, $id]);

ts_card_alloc_log(
    $id, null, 'card_balance', $ceilingUsd, $sumUsd, 'USD', null,
    'اتمام کارت «' . $card['name'] . '» — موجودی نمایشی ' . rtrim(rtrim(number_format($sumUsd, 2, '.', ''), '0'), '.') . ' دلار (سقف اصلی ' . rtrim(rtrim(number_format($ceilingUsd, 2, '.', ''), '0'), '.') . ' دلار)'
);

ts_json(200, ['ok' => true, 'display_balance_usd' => $sumUsd, 'finalized_at' => $now]);
